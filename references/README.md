# references/ — raw inspiration, with extraction notes

Rule (from the workflow): references inspire specs; specs instruct builds;
builds never look at references directly. Every artifact dropped here gets an
entry below recording what was distilled into DESIGN.md — grammar — and what
was deliberately left behind — costume.

---

## bahi-khata-spread.png
Open bahi khata register on a wooden desk. Devanagari entries, red-ruled
columns, ditto marks, boxed totals.

**Distilled into DESIGN.md v1.1:** madder-red structural rules (`--rule-red`);
two-page-spread layout with center gutter (record left / attention right);
ditto marks (") for bundle children; totals boxed with single rule above,
double rule below (C/F convention); warm paper direction.
